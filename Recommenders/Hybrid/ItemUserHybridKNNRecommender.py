...
class ItemUserHybridKNNRecommender(ItemKNNCFRecommender, UserKNNCFRecommender):
    ...
    def fit(self, topK_CF=914, shrink_CF=2000, similarity_CF='cosine', normalize_CF=True,
            feature_weighting_CF="TF-IDF", alpha=0.7,
            topK=2000, shrink=5, feature_weighting="TF-IDF",
            similarity='cosine',normalize=True,  norm_scores=True, **fit_args):
        fit_args={'topK_CF': 1123, 'shrink_CF': 462, 'similarity_CF': 'cosine', 'topK': 1263, 'shrink': 70,
                   'similarity': 'cosine', 'alpha': 0.5, 'normalize_CF': True, 'normalize': True, 'norm_scores':False}
        self.alpha = alpha
