...
def save_data(phase, Best_MAP_phase, Best_shrink_phase, Best_similarity_phase, Best_TopK_Phase):
    if (phase == "Training"):
        file = open("Testing_Results/CF_Best_Training_Old_Value.txt", "w+")
        ...
        file = open("Testing_Results/CF_Best_Validation_Old_Value.txt", "w+")
        for index in range(max_length_best):
            file.write(str(index) + ".  MAP: " + str(Best_MAP_phase[index]) + "     Shrink: " + str(Best_shrink_phase[index]) + "   topK: " + str(
                Best_TopK_Phase[index])+  "   similarity:  " + str(Best_similarity_phase[index]) + "\n")
        file.write("\nStarted at:  " + str(start_time) + "\nFinished at (Date-Time):   " + str(
        ...
        file = open("Testing_Results/CF_Best_Test_Old_Value.txt", "w+")
        for index in range(max_length_best):
            file.write(str(index) + ".  MAP: " + str(Best_MAP_phase[index]) + "     Shrink: " + str(Best_shrink_phase[index]) + "   topK: " + str(
                Best_TopK_Phase[index]) + "   similarity:  " + str(Best_similarity_phase[index]) + "\n")
        file.write("\nStarted at:  " + str(start_time) + "\nFinished at (Date-Time):   " + str(
    ...
    for similarity in tqdm(similarities):
        for topK in x_tick_rnd_topK:
            for shrink in x_tick_rnd_shrink:
                ...
                order_MAP(Best_MAP_phase=Best_MAP_training,Best_shrink_phase= Best_Shrink_training,Best_similarity_phase= Best_similarities_training,
                          Best_TopK_Phase= Best_topK_training, MAP= result_df.loc[10]["MAP"], shrink=shrink, topK=topK, similarity=similarity)
...
def validation_phase():
    ...
    for i in tqdm(range(max_length_best), desc=" validation phase..."):
            ...
            order_MAP(Best_MAP_phase=Best_MAP_validation, Best_shrink_phase=Best_Shrink_validation,Best_similarity_phase=Best_similarities_validation,
                      Best_TopK_Phase=Best_topK_validation, MAP=result_df.loc[10]["MAP"], shrink=Best_Shrink_training[i], topK=Best_topK_training[i],
...
#x_tick_rnd_topK = [353, 355, 335, 110, 230, 80, 400]
x_tick_rnd_topK=[353]

#x_tick_rnd_shrink = [488, 439, 448, 421, 176, 90, 300]
x_tick_rnd_shrink=[488]
import os
